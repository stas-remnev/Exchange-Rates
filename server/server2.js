...
function requestData(ld,lm,ly){
 http.get("http://www.cbr.ru/scripts/XML_dynamic.asp?date_req1=" + ld + "/" + lm +"/" + ly +"&date_req2=" + d + "/" + m + "/" + y + "&VAL_NM_RQ=R01235", function (res) {
    str = '';
    ...
    res.on('end', function () {
        ...
        for (var i = 0; i < k; i++) {
            ...
            arr3[i] = arr3[i].charAt(4) + arr3[i].charAt(5) + arr3[i].charAt(6) + arr3[i].charAt(7)  + "-" + arr3[i].charAt(2) + arr3[i].charAt(3) + "-" + arr3[i].charAt(0) + arr3[i].charAt(1);
        }
...
app.listen(8080, function () {
    ...
    fs.readFile("lastrequest.txt", 'utf8', function(err, data){
        if (data != (now.toLocaleDateString()).substring(0,10)){
            val = new Date(data); 
        ...
        fs.writeFile("lastrequest.txt", (now.toLocaleDateString()).substring(0,10), function (err) {
            if (err) throw err;
        });
    }
});
...
function getCurrencyData(from, to) {
    ...
    console.log(data);
    var pos = data.findIndex(function (element){return element.date == from});
    for(var i=pos;i++;i<=pos+num){
